...
# sort the contours from left to right and initalize the bounding box point colors
(cnts,_) = contours.sort_contours(cnts)
colors = ((0,0,255),(240,0,159),(255,0,0),(255,255,0))

# loop over the contours individually
for (i,c) in enumerate(cnts):
    # if the contour is not sifficiently large (due to "noise" in the edge detection process), ignore it
    ...
    # order the points in the contour such that they appear in the top left, top right, bottom right, and
    # bottom left order, then draw the outline of the rotated bounding box
    rect = order_points_old(box)

    # check to see if the new method should be used for ordering the coordinates
    if args['new'] > 0:
    ...
    cv2.putText(image,'Object #{}'.format(i+1),(int(rect[0][0]-15)),cv2.FONT_HERSHEY_SIMPLEX,0.55,(255,255,255),2)
